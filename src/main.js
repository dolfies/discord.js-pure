    ...
    _corePlugins: [
        {
            ...
            start: function () {
                Discord.patchModule(
                    "instead",
                    Discord.findModule.byProps("track"),
                    "track",
                    function () {
                        return;
                    },
                ...
                Discord._originalFunctions._DiscordSentry =
                    window.DiscordSentry;
                window.DiscordSentry = {};
            },
            stop: function () {
                Discord.unpatchByPatchSignature(
                    Discord.findModule.byProps("track"),
                    "track",
                    "DoNotTrack.1",
                );
        ...
        },
        {
            ...
            start: function () {
                Discord.patchModule(
                    "instead",
                    Discord.findModule.byProps("startTyping"),
                    "startTyping",
                    function () {
                        return;
                    },
...
(Logger = {
    ...
    log(toLog) {
        if (this._log == true) {
            console.log(
                `%c[discordjs-pure] (LOGGER)%c ${toLog}`,
                "color: #9e0700",
                "color: #ffffff",
            ); // [discordjs-pure] (LOGGER) is red, the text that's logged is white
        }
    },
}),
    (Utils = {
        random(min, max) {
            /* Utils.random(minimum_value, maximum_value); */ // lets you get a random value by supplying a minimum & maximum
...
Discord = {
    ...
    _createMessage(content, embeds) {
        return this.findModule
            .byDisplayName("createBotMessage")
            .createBotMessage(this.getCurrentChannelID(), content, embeds);
    },

    _sendLocalMessage(channel, message) {
        return this.findModule
            .byProps("receiveMessage")
            .receiveMessage(channel, message);
    },

    _getCurrentChannelID() {
        return this.findModule
            .byProps("getLastSelectedChannelId", "getChannelId")
            .getChannelId();
    },
    ...
    findModule: {
        byDisplayName(MODULE) {
            for (const m of Object.keys(Discord._cachedWebpack)
                .map((x) => Discord._cachedWebpack[x].exports)
                .filter((x) => x)) {
        ...
        async asFunction(n, fa) {
            ...
            o =
                o ||
                Object.values(Discord._cachedWebpack).find(
                    (m) => m?.exports?.default && m.exports.default[n],
                );
            typeof o?.exports?.default[n] !== "undefined"
                ? (f = o?.exports?.default[n])
                : (f = o?.exports[n]);
        ...
        fuzzy(n, b) {
            (d = typeof b === "undefined" ? true : b),
                ...
                (m = new Array());
            m.push(
                ...Object.values(Discord._cachedWebpack).filter(
                    (m) =>
                        m?.exports &&
                        ((m?.exports?.default &&
    ...
    patchModule(type, module, func, callback, signature) {
        ...
        if (!type) {
            throw new TypeError(
                "You must specify what type of patch to perform.",
            );
        }
        if (!module) {
        ...
        if (!signature) {
            throw new TypeError(
                "You must specify a signature/name for your patch.",
            );
        }
    ...
    _removePatchesFromList(func, signature) {
        for (var i = 0; i < this._currentPatches.length; i++) {
            // this for loop will remove every patch with the signature that you wanna remove
            let patch = this._currentPatches[i];
            if (patch.signature == signature) {
    ...
    unpatchByPatchSignature(module, func, signature) {
        ...
        if (!signature) {
            throw new TypeError(
                "You must specify a patch signature to unpatch by.",
            );
        }
    ...
    getToken() {
        // this function has two fallbacks, so it should always work.
        ...
        if (token.includes(".") == false) {
            if (window.localStorage != undefined) {
                ...
                if (!popup) {
                    return alert(
                        "[discordjs-pure] The popup required to grab the token was blocked! Allow popups or this won't work. After you allow popups, reload this page and re-paste this script.",
                    );
                }
...
Client = class Client {
    ...
    get user() {
        // Property is better because it's often overwritten by Discord
    ...
    startLurking(id) {
        // lets you do the 'guild preview' process for a supplied guild-id
        if (!id) {
            throw new TypeError("You must specify a guildID to preview.");
        }
        Discord.findModule
            .byDisplayName("startLurking")
            .startLurking(id, {}, undefined);
    }

    stopLurking(id) {
        // should always be called after you're done with lurking
        if (!id) {
            throw new TypeError(
                "You must specify a guildID to stop previewing.",
            );
        }
    ...
    connect() {
        ...
        Discord.findModule.byDisplayName(
            "Dispatcher",
        ).Dispatcher.prototype._interceptor = function (e) {
